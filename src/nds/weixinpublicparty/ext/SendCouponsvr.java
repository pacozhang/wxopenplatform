	...
	protected String doIt() throws Exception {
		...
		for (int i = 0; i < sendcouponinfo.size(); i++) {
			...
			if (ins!=null) {
				...
				if (usertype == 3) {
					...
					if (jo!=null&&jo.optInt("errCode", -1) == 0) {// 线下发劵成功
						String updateState = " update wx_sendcoupondetail scdtl set scdtl.sync_log='同步成功',scdtl.sync_state=2,scdtl.sync_count=nvl(scdtl.sync_count,0)+1 where scdtl.id=" + scdtlid;
						QueryEngine.getInstance().executeUpdate(updateState);
						ticketno = jo.optJSONObject("result").optJSONObject("data").optString("code");
						log.debug("线下操作成功");
					} else {
						String updatelog = " update wx_sendcoupondetail scdtl set scdtl.sync_log='同步失敗',scdtl.sync_count=nvl(scdtl.sync_count,0)+1 where scdtl.id=" + scdtlid;
			...
			if (jo2.optInt("code", -1) == 0) {
				...
				log.debug("线下操作失败222222222222222222222");
