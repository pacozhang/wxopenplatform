...
public class ScanSotreRqcodeCommand extends Command{
	...
	public ValueHolder execute(DefaultWebEvent event) throws NDSException,
		...
		try {
			QueryEngine.getInstance().executeUpdate("update wx_vip v set v.store=?,v.nowstore=?,v.modifieddate=sysdate where v.wechatno=? and v.ad_client_id=? and v.store is null", new Object[] {storeid,storeid,messagejo.optString("ToUserName"),ad_client_id});
			vh.put("code", "0");
			vh.put("message", "update vip store_id success");
		} catch (Exception e) {
			logger.debug("update vip store error->"+e.getLocalizedMessage());
			vh.put("code", "-1");
			vh.put("message", "update vip store error->"+e.getLocalizedMessage());
			return vh;
		}
		return vh;
	}
